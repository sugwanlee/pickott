...
def get_genre_map():
    ...
    if response.status_code == 200:
        return {
            genre["id"]: genre["name"] for genre in response.json().get("genres", [])
        }
...
def fetch_movie_data(page):
    ...
    response = session.get(
        BASE_URL, params={"api_key": API_KEY, "language": LANGUAGE, "page": page}
    )
    if response.status_code == 200:
...
def fetch_watch_providers(movie_id):
    ...
    if response.status_code == 200:
        ...
        return (
            ", ".join(provider["provider_name"] for provider in watch_data)
            if watch_data
            else ""
        )
    return ""
...
def process_movie(movie, genre_map):
    ...
    return {
        ...
        "genres": ", ".join(
            [genre_map.get(genre_id, "기타") for genre_id in movie["genre_ids"]]
        ),
